...
getFrfsRoutes (_personId, _mId) mbEndStationCode mbStartStationCode _city _vehicleType = do
  ...
  integratedBPPConfig <- QIBC.findByDomainAndCityAndVehicleCategory (show Spec.FRFS) merchantOpCity.id (frfsVehicleCategoryToBecknVehicleCategory _vehicleType) (Just DIBC.APPLICATION) >>= fromMaybeM (InternalError "integratedBPPConfig is missing")
...
getFrfsStations (_personId, mId) mbCity mbEndStationCode mbOrigin mbRouteCode mbStartStationCode vehicleType_ = do
  ...
  integratedBPPConfig <- QIBC.findByDomainAndCityAndVehicleCategory (show Spec.FRFS) merchantOpCity.id (frfsVehicleCategoryToBecknVehicleCategory vehicleType_) (Just DIBC.APPLICATION) >>= fromMaybeM (InternalError "integratedBPPConfig is missing")
...
postFrfsSearchHandler (mbPersonId, merchantId) mbCity vehicleType_ FRFSSearchAPIReq {..} frfsRouteDetails mbPOrgTxnId mbPOrgId mbJourneyRouteDetails platformType = do
  ...
  mbCity' <- fromMaybeM (InternalError "mbCity does not exist") mbCity
  --mbCity' <- mbCity >>= fromMaybe (InternalError $ "mbCity does not exist")
  ...
  merchantOperatingCity <- CQMOC.findByMerchantIdAndCity merchant.id mbCity' >>= fromMaybeM (MerchantOperatingCityNotFound $ "merchant-Id-" <> merchant.id.getId <> "-city-" <> show mbCity')
  integratedBPPConfig <- QIBC.findByDomainAndCityAndVehicleCategory (show Spec.FRFS) merchantOperatingCity.id (frfsVehicleCategoryToBecknVehicleCategory vehicleType_) (Just platformType) >>= fromMaybeM (InternalError "integratedBPPConfig is missing")
  (fromStation, toStation) <-
    case mbCity of
      ...
      Nothing -> do
        ...
        fromStationInfo <- QStation.findByStationCode fromStationCode integratedBPPConfig.id >>= fromMaybeM (InvalidRequest "Invalid from station id")
        toStationInfo <- QStation.findByStationCode toStationCode integratedBPPConfig.id >>= fromMaybeM (InvalidRequest "Invalid to station id")
        return (fromStationInfo, toStationInfo)
  route <-
    maybe
      (pure Nothing)
  ...
  CallExternalBPP.search merchant merchantOperatingCity bapConfig searchReq frfsRouteDetails platformType
...
getFrfsAutocomplete (_, mId) mbInput mbLimit mbOffset opCity origin vehicle = do
  ...
  integratedBPPConfig <- QIBC.findByDomainAndCityAndVehicleCategory (show Spec.FRFS) merchantOpCity.id (frfsVehicleCategoryToBecknVehicleCategory vehicle) (Just DIBC.APPLICATION) >>= fromMaybeM (InternalError "integratedBPPConfig is missing")
  case mbInput of
    Nothing -> do
